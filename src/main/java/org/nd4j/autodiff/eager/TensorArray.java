...
public class TensorArray implements INDArray {
    SDVariable data;

    public TensorArray(SDVariable variable) {
        data = variable;
    }

    private TensorArray newTensor(INDArray constant) {
        return new TensorArray(data.getSameDiff().constant(constant));
    }

    @Override
    public String shapeInfoToString() {
        return data.getArr().shapeInfoToString();
    }

    @Override
    public DataBuffer shapeInfoDataBuffer() {
        return data.getArr().shapeInfoDataBuffer();
    }

    @Override
    public LongBuffer shapeInfo() {
        return data.getArr().shapeInfo();
    }

    @Override
    public boolean isView() {
        return data.getArr().isView();
    }

    @Override
    public boolean isSparse() {
        return data.getArr().isSparse();
    }

    @Override
    public boolean isCompressed() {
        return data.getArr().isCompressed();
    }

    @Override
    public void markAsCompressed(boolean reallyCompressed) {
        data.getArr().markAsCompressed(reallyCompressed);
    }

    @Override
    public int rank() {
        return data.getArr().rank();
    }

    @Override
    public int stride(int dimension) {
        return data.getArr().stride(dimension);
    }

    @Override
    public int elementWiseStride() {
        return data.getArr().elementWiseStride();
    }

    @Override
    public double getDoubleUnsafe(long offset) {
        return data.getArr().getDoubleUnsafe(offset);
    }

    @Override
    public String getString(long index) {
        return data.getArr().getString(index);
    }

    @Override
    public INDArray putScalarUnsafe(long offset, double value) {
        return data.getArr().putScalarUnsafe(offset, value);
    }

    @Override
    public long vectorsAlongDimension(int dimension) {
        return data.getArr().vectorsAlongDimension(dimension);
    }

    @Override
    public INDArray vectorAlongDimension(int index, int dimension) {
        return data.getArr().vectorAlongDimension(index, dimension);
    }

    @Override
    public long tensorsAlongDimension(int... dimension) {
        return data.getArr().tensorsAlongDimension(dimension);
    }

    @Override
    public INDArray tensorAlongDimension(long index, int... dimension) {
        return data.getArr().tensorAlongDimension(index, dimension);
    }

    @Override
    public INDArray cumsumi(int dimension) {
        return data.getArr().cumsumi(dimension);
    }

    @Override
    public INDArray cumsum(int dimension) {
        return data.getArr().cumsum(dimension);
    }

    @Override
    public INDArray assign(INDArray arr) {
        return data.getArr().assign(arr);
    }

    @Override
    public INDArray assignIf(INDArray arr, Condition condition) {
        return data.getArr().assignIf(arr, condition);
    }

    @Override
    public INDArray replaceWhere(INDArray arr, Condition condition) {
        return data.getArr().replaceWhere(arr, condition);
    }

    @Override
    public INDArray putScalar(long i, double value) {
        return data.getArr().putScalar(i, value);
    }
    ...
    public INDArray putScalar(long row, long col, double value) {
        return data.getArr().putScalar(row, col, value);
    }

    @Override
    public INDArray putScalar(long dim0, long dim1, long dim2, double value) {
        return data.getArr().putScalar(dim0, dim1, dim2, value);
    }

    @Override
    public INDArray putScalar(long dim0, long dim1, long dim2, long dim3, double value) {
        return data.getArr().putScalar(dim0, dim1, dim2, dim3, value);
    }

    @Override
    public INDArray lt(Number other) {
        SDVariable lt = data.lt(other.doubleValue());

        return new TensorArray(lt);
    }

    @Override
    public INDArray putScalar(int[] indexes, float value) {
        return data.getArr().putScalar(indexes, value);
    }
    ...
    public INDArray eps(Number other) {
        ...
        return data.getArr().eps(other);
    }

    @Override
    public INDArray eq(Number other) {
        return new TensorArray(data.eq(other.doubleValue()));
    }

    @Override
    public INDArray gt(Number other) {
        return new TensorArray(data.gt(other.doubleValue()));
    }

    @Override
    public INDArray gte(Number other) {
        return new TensorArray(data.gte(other.doubleValue()));
    }

    @Override
    public INDArray lte(Number other) {
        return new TensorArray(data.lte(other.doubleValue()));
    }

    @Override
    public INDArray lt(INDArray other) {
        SDVariable constant = data.getSameDiff().constant(other);
        return new TensorArray(data.lt(constant));
    }

    @Override
    public INDArray eps(INDArray other) {
        return newTensor(data.getArr().eps(other));
    }

    @Override
    public INDArray neq(Number other) {
        return new TensorArray(data.neq(other.doubleValue()));
    }

    @Override
    public INDArray neq(INDArray other) {
        return new TensorArray(data.neq(data.getSameDiff().constant(other)));
    }

    @Override
    public INDArray eq(INDArray other) {
        return new TensorArray(data.eq(data.getSameDiff().constant(other)));
    }

    @Override
    public INDArray gt(INDArray other) {
        return new TensorArray(data.gt(data.getSameDiff().constant(other)));
    }

    @Override
    public INDArray isInfinite() {
        return newTensor(data.getArr().isInfinite());
    }

    @Override
    public INDArray isNaN() {
        return newTensor(data.getArr().isNaN());
    }

    @Override
    public INDArray neg() {
        return new TensorArray(data.neg());
    }
    ...
    public INDArray rdiv(Number n) {
        return new TensorArray(data.rdiv(n.doubleValue()));
    }

    @Override
    public INDArray rdivi(Number n) {
        data = data.rdiv(n.doubleValue());
        return this;
    }

    @Override
    public INDArray rsub(Number n) {
        SDVariable rsub = data.rsub(n.doubleValue());
        return new TensorArray(rsub);
    }

    @Override
    public INDArray rsubi(Number n) {
        data = data.rsub(n.doubleValue());
    ...
    public INDArray div(Number n) {
        return new TensorArray(data.div(n.doubleValue()));
    }

    @Override
    public INDArray divi(Number n) {
        data = data.div(n.doubleValue());
        return this;
    }

    @Override
    public INDArray mul(Number n) {
        return new TensorArray(data.mul(n.doubleValue()));
    }

    @Override
    public INDArray muli(Number n) {
        data = data.mul(n.doubleValue());
        return this;
    }

    @Override
    public INDArray sub(Number n) {
        return new TensorArray(data.sub(n.doubleValue()));
    }

    @Override
    public INDArray subi(Number n) {
        data = data.sub(n.doubleValue());
        return this;
    }

    @Override
    public INDArray add(Number n) {
        return new TensorArray(data.add(n.doubleValue()));
    }

    @Override
    public INDArray addi(Number n) {
        data = data.add(n.doubleValue());
        return this;
    }
